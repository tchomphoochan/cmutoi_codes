...
int n, pw[N], dp[N][3], ans = 0;
vector<int> G[N];

void max3(int &m1, int &m2, int &m3, int x)
{
...
void solve(int u, int p)
{
    ...
    fora(v, G[u]) if (v != p) {
        solve(v, u);
        setmax(dp[u][1], pw[u] + max(dp[v][1], dp[v][2]));
        max3(m1, m2, m3, dp[v][1]);
        max3(n1, n2, n3, max(dp[v][1], dp[v][2]));
    }
    dp[u][2] = m1 + m2 + pw[u];
    dp[u][0] = max(m1+m2+m3, n1+n2) + pw[u];
    setmax(ans, dp[u][0]);
}
...
int _main()
{
    ...
    solve(0, 0);
    W(ans);
    //forn(i, n) W(dp[i][0], dp[i][1], dp[i][2]);
