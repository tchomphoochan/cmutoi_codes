...
struct Matrix {
    ...
    Matrix operator*(Matrix &rhs) {
        ...
        int r = rhs.m;
        Matrix ans(n, r);
        forn(i, n) forn(j, r) forn(k, m)
            ans[i][j] = add(ans[i][j], mul(mat[i][k], rhs[k][j]));
        return ans;
    }
};
...
int cline(int p, int i=0)
{
    if (i == n) return 1;
    int ans = cline(p, i+1);
    if (i != n-1 && !hasbit(p, i) && !hasbit(p, i+1))
        ans = add(ans, cline(p, i+2));
    return ans;
}

void cbox(int p, int ans[], int i=0, int v=0)
{
    if (i==n) {
        ++ans[v];
        return;
...
int _main()
{
    ...
    Matrix st(1<<n, 1);
    forn(i, 1<<n) st[i][0] = cline(i);
    Matrix mat(1<<n, 1<<n);
    forn(i, 1<<n) {
        int ans[1<<n] = {};
        cbox(i, ans);
        forn(j, 1<<n) mat[j][i] = ans[j];
    }

    Matrix ans(1<<n, 1<<n);
    forn(i, 1<<n) ans[i][i] = 1;
    --m;
    while (m) {
        if (m&1) ans = mat*ans;
        m >>= 1;
