...
int _main()
{
    ...
    while (q--) {
        ...
        forn(i, m) {
            char c = t[i]-'a';
            auto it = upper_bound(all(occ[c]), *prev(has.end()));
            if (it != occ[c].end()) {
                has.insert(*it);
            }
            for (auto it = --has.end(); it != has.begin(); --it) {
                auto f = upper_bound(all(occ[c]), *prev(it));
                if (f == occ[c].end() || *f >= *it) continue;
                auto nit = has.insert(*f).F;
                has.erase(it);
                it = nit;
