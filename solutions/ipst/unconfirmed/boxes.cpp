...
{
    if (i > j) return 0;
    if (dp[i][j][c] != 0) return dp[i][j][c];
    ll ans = (ll)c*c*c + solve(i+1, j, 1);
    forx(x, cpos[i]+1, call[col[i]].size()-1) {
        int k = call[col[i]][x];
        if (k > j) break;
        setmax(ans, solve(i+1, k-1, 1) + solve(k, j, c+1));
    }
    return dp[i][j][c] = ans;
}
