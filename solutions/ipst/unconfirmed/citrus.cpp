...
int _main()
{
    ...
    for1(l, k) for1(i, k) for1(j, k) if (i != j && j != l && i != l)
        setmin(A[i][j], A[i][l]+A[l][j]);
    for1(i, k) R(cost[i]);

    for1(s, n) for1(l, n-s+1) {
        int r = l+s-1;
        for1(c, k) {
            dp[l][r][c] = INF;
            if (s == 1) {
                if (c == stc[l]) dp[l][r][c] = 0;
                continue;
            }
            forx(x, l, r-1) {
                int nc = c-1;
                if (c-1 == 0) nc = k;
                int v = dp[l][x][nc] + dp[x+1][r][nc] + cost[nc];
                setmin(dp[l][r][c], v);
            }
        }
        for1(c, k) for1(d, k)
            setmin(dp[l][r][c], dp[l][r][d] + A[d][c]);
    }
    int ans = INF;
    for1(c, k) setmin(ans, dp[1][n][c]);
    W(ans);
