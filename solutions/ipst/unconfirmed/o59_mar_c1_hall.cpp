...
const int N = 10010;
const int PB = 98765431;
const pii PP = pii(PB, PB);
const int MOD1 = 1e9+7;
const int MOD2 = 1e9+123;

pii add(pii a, pii b) {
    pii c = {a.F+b.F, a.S+b.S};
    while (c.F >= MOD1) c.F -= MOD1;
    while (c.F < 0) c.F += MOD1;
    while (c.S >= MOD2) c.S -= MOD2;
    while (c.S < 0) c.S += MOD2;
    return c;
}
pii rem(pii a, pii b) { return add(a, pii(-b.F, -b.S)); }
pii mul(pii a, pii b) { return pii((a.F*1ll*b.F)%MOD1, (a.S*1ll*b.S)%MOD2); }
...
bool check(int len)
{
    ...
    pii h(0, 0), rm(1, 1);
    for1(i, len-1) {
        h = add(mul(h, PP), pii(A[i], A[i]));
        rm = mul(rm, PP);
    }
    //W("-------------");
    //W("rm = ", rm.F, rm.S);
    bool first = true;
    for (int i = len, j = 1; first || i != len; i = i%n+1, j = j%n+1) {
        h = add(mul(h, PP), pii(A[i], A[i]));
        //W("at", j, i, "equal", h.F, h.S);
        if (hsh.count(h)) return true;
        hsh.insert(h);
        h = rem(h, mul(rm, pii(A[j], A[j])));
        //W("rem", h.F, h.S);
        first = false;
    }
    return false;
