 ...
 * Solution uses tries. Observe how the answers interact with tries.
 */
...
node *add(node *root, int idx)
{
    if (!root) root = new node;
    if (adding[idx] == '\0')
        root->end = true;
    else
        root->M[adding[idx]] = add(root->M[adding[idx]], idx+1);
    return root;
}

int paired = 0;
int traverse(node *root, node *realroot)
{
    if (root == NULL) return 0;
    int cnt = 0;
    for (auto x : root->M) {
        cnt += traverse(x.second, realroot);
    }
    if (root->end)
        ++cnt;
    if (cnt >= 2 && root != realroot) {
        ++paired;
        cnt -= 2;
    }
    return cnt;
...
int _main()
{
    ...
    for (int t = 1; t <= T; ++t) {
        ...
        forn(i, n) {
            scanf(" %s", adding);
            reverse(adding, adding+strlen(adding));
            root = add(root, 0);
        }
        traverse(root, root);
        printf("Case #%d: %d\n", t, paired*2);
