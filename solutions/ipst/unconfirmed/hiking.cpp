...
bool dfs(int u, int p)
{
    ...
    fora(v, G[u]) {
        if (!disc[v]) {
            ++c;
            bool isp = u == n;
            if (dfs(v, u)) isp = inp = true;
            if (isp && ((p == 0 && c > 1) || (p != 0 && low[v] >= disc[u]))) {
                if (!sap[u])
                    lsap.pb(u);
                sap[u] = true;
            }
...
int bfs(int s, int t)
{
    ...
    while (!Q.empty()) {
        ...
        if (vis[u]) continue;
        vis[u] = true;
        if (u == s || !sap[u]) fora(v, G[u]) if (!vis[v])
            Q.emplace(v, d+1);
    }
    assert(false);
    return -1;
...
int _main()
{
    ...
    dfs(1, 0);
    if (!lsap.empty() && lsap.back() == 1)
        lsap.pop_back();
    reverse(all(lsap));
    if (!lsap.empty() && lsap.back() == n)
        lsap.pop_back();
    if (lsap.size() == 0) {
        W("0");
        return 0;
