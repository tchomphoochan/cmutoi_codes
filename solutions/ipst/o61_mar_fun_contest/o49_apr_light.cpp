...
struct node {
    ...
    node *lc, *rc;
    node() { }
    node(int l, int r, int s) : l(l), r(r), s(s), f(false), lc(NULL), rc(NULL) { }
    void prop() {
        if (f) {
            f = false;
            s = (r-l+1)-s;
            if (lc) lc->f = !lc->f, rc->f = !rc->f;
        }
    }
};
...
void update(node *p, int l, int r)
{
    ...
    int m = (p->l+p->r)/2;
    if (!p->lc) {
        p->lc = new node(p->l, m, (p->s+1)/2);
        p->rc = new node(m+1, p->r, p->s/2);
    }
    update(p->lc, l, r);
    update(p->rc, l, r);
    p->s = p->lc->s + p->rc->s;
