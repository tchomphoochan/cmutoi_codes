...
{
    for (t[p += c].insert(x); p >>= 1; )
        t[p].insert(x);
}

int query(int l, int r, pii x)
{
    int ans = 0;
    for (l += c, r += c+1; l < r; l >>= 1, r >>= 1) {
        if (l&1) ans += t[l++].order_of_key(x);
        if (r&1) ans += t[--r].order_of_key(x);
    }
    return ans;
}

int pos(int x) { // more toward the left
    return upper_bound(all(coord), x)-1 - coord.begin();
}

int pos2(int x) { // more toward the right
...
int _main()
{
    ...
    while (m--) {
        int l, r;
        R(l, r);
        int al = (upper_bound(A, A+n, r)-A) - (upper_bound(B, B+n, l-1)-B);
        int start = (upper_bound(A, A+n, r)-1) - (upper_bound(A, A+n, l-1)-1);
        int end = (upper_bound(B, B+n, r)-1) - (upper_bound(B, B+n, l-1)-1);
        int both = query(pos2(l), pos(r), pii(r, INF));
        int none = al-(start+end-both);
        W(al, start-both, end-both, both, none);
    }
