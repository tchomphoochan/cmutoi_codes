...
int n, k, c;
ll ans = 0;
int deg[N], par[N], len[N], ft[N];
vector<pii> G[N];
vector<int> coord, st;

int get(int i) { int ans = 0; for (; i > 0; i -= i&-i) ans += ft[i]; return ans; }
void upd(int i, int x) { for (; i <= c; i += i&-i) ft[i] += x; }

int pos(int x) { return upper_bound(all(coord), x)-coord.begin() - 1; }
...
void dfs1(int u, int w, int t) {
    if (t == 0) coord.pb(w);
    else if (t == 1) ans += get(pos(k-w));
    else if (t == 2) upd(pos(w), 1);
    fora(v, G[u]) if (v.F != par[u])
        dfs1(v.F, w+v.S, t);
}
...
int _main() {
    ...
    int sp = 0;
    for1(i, n) if (deg[i] > 2) sp = i;
    if (sp == 0) for1(i, n) if (deg[i] == 1) sp = i;
    dfs(sp, 0, 0);
    fora(x, st) {
        if (x == sp) continue;
        int u=x, v=x, i=0, j=0, w=0;
        do {
            ++i;
            w += len[v];
            v = par[v];
