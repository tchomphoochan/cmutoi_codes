...
int dep[N*2], sz[N*2], cnt[N*2], par[N*2][LN];
...
int getans(int arr[], int u, int v)
{
    int l = lca(u, v);
    return arr[u] + arr[v] - arr[l] - arr[par[l][0]];
}
...
int _main()
{
    ...
    for1(u, cnt) ::cnt[u] = ::cnt[u] == 0 ? 1 : 0;
    /*for1(i, vc.size()) W("comp", i, "has", vc[i-1]);
    for1(u, n) W("node", u, "id", id[u]);
    for1(u, cnt) W(u, sz[u], ::cnt[u], ":", G[u]);*/
    dfs2(1, 0);
    //for1(u, cnt) W(u, sz[u], ::cnt[u]);
    while (q--) {
        int u, v;
        R(u, v);
        int p1 = getans(sz, id[u], id[v]);
        int p2 = getans(::cnt, id[u], id[v]);
        W(p1-(p2-1)-2);
    }
