...
int _main()
{
    ...
    while (true) {
        ...
        //W("got char", ans, "and left with k=", k);
        ansstr.pb(ans);
        sum = 0;
        fora(c, can) {
            if (s[c.S] < ans)
                k -= cnt[c.F][c.S];
            else if (s[c.S] == ans && dp[c.F][c.S])
                ++sum;
        }
        //W("there are", sum, "strings");
        if (sum > k)
            break;
        k -= sum;
        //W("left with", "k=", k);
        vector<pii> nxt;
        fora(c, can) if (s[c.S] == ans && c.S+1 <= n)
            nxt.eb(c.F, c.S+1);
        can = nxt;
    }
