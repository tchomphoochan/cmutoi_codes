...
int x[N], y[N], dp[N];
int at[N]; // 1 = above, 0 = diag, -1 = below

dbl slope(dbl x1, dbl y1, dbl x2, dbl y2)
{
    return x2-x1 == 0 ? INF : (y2-y1)/(x2-x1);
}
...
int _main()
{
    ...
    for1(i, n) if (at[i] != 0) {
        ...
        for1(j, i-1) {
            if (at[i] == 1 && at[j] == 1)
                pu.pb(x[i] < x[j] ? pii(i, j) : pii(j, i));
            if (at[i] == -1 && at[j] == -1)
                pd.pb(x[i] > x[j] ? pii(i, j) : pii(j, i));
        }
    }
    sort(all(pu), [&] (pii a, pii b) {
        dbl sa = slope(x[a.F], y[a.F], x[a.S], y[a.S]);
        dbl sb = slope(x[b.F], y[b.F], x[b.S], y[b.S]);
        if (sa != sb) return sa > sb;
        return x[a.F] < x[b.F];
    });
    sort(all(pd), [&] (pii a, pii b) {
        dbl sa = slope(x[a.F], y[a.F], x[a.S], y[a.S]);
        dbl sb = slope(x[b.F], y[b.F], x[b.S], y[b.S]);
        if (sa != sb) return sa > sb;
        return x[a.F] > x[b.F];
    ...
    forx(i, 0, n+1) dp[i] = -INF;
    dp[0] = 0;
    fora(p, pu) setmax(dp[p.S], dp[p.F]+1);
    int gou = max(dp[n+1]-1, 0);

    forx(i, 0, n+1) dp[i] = -INF;
    dp[n+1] = 0;
    fora(p, pd) setmax(dp[p.S], dp[p.F]+1);
    int god = max(dp[0]-1, 0);
    
    W(max(gou+god, max(gou+cntd, god+cntd)));
