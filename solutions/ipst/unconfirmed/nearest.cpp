...
int add(set<int> &ind, multiset<int> &diff, int x)
{
    auto rit = ind.lower_bound(x);
    int l = *prev(rit);
    int r = *rit;
...
int remove(set<int> &ind, multiset<int> &diff, int x)
{
    auto xit = ind.find(x);
    int l = *prev(xit);
    int r = *next(xit);
...
int _main()
{
    ...
    forn(i, m) {
        int k = K[i], a = A[i];
        int p = pos(col[k]);
        remove(ind[p], diff[p], k);
        col[k] = a;
        p = pos(col[k]);
        ll v = add(ind[p], diff[p], k);
        ans += v;
    }
    W(ans);
