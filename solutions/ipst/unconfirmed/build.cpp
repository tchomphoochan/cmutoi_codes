...
int _main()
{
    ...
    rof1(i, n) R(A[i]);
    rof1(i, n) R(B[i]);
    for1(i, n) forn(j, n+1) dp[i][j] = INF;
    for1(i, n) forn(j, n+1) {
        if (j != 0) // cut above (leave below)
            setmin(dp[i][j], A[i] + dp[i-1][j-1]);
        // cut below (leave above) and cut cross links
        setmin(dp[i][j], B[i] + dp[i-1][j] + c*j);
        if (j != 0)
            setmin(dp[i][j], A[i] + B[i] + dp[i-1][j-1]); // cut both, but leave chances of entering
        setmin(dp[i][j], A[i] + B[i] + c*j + dp[i-1][j]); // cut both, leave no chances
    }
    ll ans = INF;
    forn(i, n+1) setmin(ans, dp[n][i]);
    W(ans);
