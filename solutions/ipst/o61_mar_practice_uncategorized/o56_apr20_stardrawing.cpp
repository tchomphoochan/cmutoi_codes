...
vector<int> num;
bool cyc = false;
void dfs(int u, int l=0, int p=0, int s=0)
{
    if (vis[u]) {
        cyc = true;
        num.push_back(l);
...
int _main()
{
    ...
    for1(i, n) if (!vis[i]) {
        ...
        if (num.size() == 1) {
            found.insert({0, num[0]+1, 0, 0});
            //W("insert", 0, num[0]+1, 0, 0);
        } else if (num.size() == 2) {
            assert(num[1] >= num[0]);
            found.insert({2, num[1], num[0], 0});
            //W("insert", 2, num[1], num[0], 0);
        } else if (num.size() == 3) {
            sort(all(num));
            found.insert({1, num[0], num[1], num[2]});
            //W("insert", 1, num[0], num[1], num[2]);
        } else {
            assert(num.size() == 0);
        }
        num.clear();
    ...
    for1(i, n) if (!vis[i]) {
        ...
        cyc = false;
        dfs(i);
        found.insert({2, num[0], num[0], 0});
        assert(num.size() == 1);
        //W("insert", 2, num[0], num[0], 0);
        num.clear();
    }
    W(found.size());
