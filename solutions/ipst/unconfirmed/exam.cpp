...
int n, m, A[N], ft[N], ans[M];
map<int, int> p;
vector<pii> todo[N];

void upd(int i, int x) { for (; i > 0; i -= i&-i) ft[i] += x; }
int get(int i) { int ans = 0; for (; i <= n; i += i&-i) ans += ft[i]; return ans; }
...
int _main()
{
    ...
    for1(i, n) {
        if (p[A[i]] != 0) upd(p[A[i]], -1);
        upd(i, 1), p[A[i]] = i;
        while (!todo[i].empty()) {
            pii x = todo[i].back();
            todo[i].pop_back();
            ans[x.S] = (i-x.F+1) - get(x.F);
        }
    }
    for1(i, m) W(ans[i]);
