 ...
 * This passes small test case only (+8pts -23pts).
 * The solution is naive exhaustive search.
 */
...
void dfs(int i, int j, int cnt=n*m-1)
{
    if (cnt == 0) {
        sol.eb(i, j);
        ok = true;
    ...
    for1(x, n) {
        for1(y, m) {
            if (!used[x][y] && i!=x && j!=y && i+j!=x+y && i-j!=x-y) {
                dfs(x, y, cnt-1);
                if (ok) break;
            }
        }
