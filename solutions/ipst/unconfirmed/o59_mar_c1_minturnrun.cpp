...
int _main()
{
    ...
    #define bound(x, y) (x >= 1 && x <= n && y >= 1 && y <= m && B[x][y] != '#' && !vis[x][y])
    deque<node> Q;
    Q.pb({s,1,0,0});
    while (!Q.empty()) {
        node u = Q.front();
        ...
        vis[u.x][u.y] = true;
        int up = u.dir == 0 ? 1 : u.dir == 1 ? 0 : -1;
        int down = u.dir == 0 ? 1 : u.dir == 1 ? -1 : 0;
        int right = u.dir == 0 ? 0 : 1;
        if (up != -1 && bound(u.x-1, u.y)) {
            if (up == 0) Q.pf({u.x-1, u.y, 1, u.dist});
            else Q.pb({u.x-1, u.y, 1, u.dist+1});
        }
        if (down != -1 && bound(u.x+1, u.y)) {
            if (down == 0) Q.pf({u.x+1, u.y, 2, u.dist});
            else Q.pb({u.x+1, u.y, 2, u.dist+1});
        }
        if (bound(u.x, u.y+1)) {
            if (right == 0) Q.pf({u.x, u.y+1, 0, u.dist});
            else Q.pb({u.x, u.y+1, 0, u.dist+1});
        }
    }
    W(-1);
